Prebuilt AlertOps and New Relic Integration Improves Your Software Performance Monitoring
========

New Relic is a popular performance monitoring tool that tracks almost any performance metric you could want. New Relic visualizes all your data in a centralized dashboard so you can quickly identify where the performance issues are.

But, New Relic is a monitoring tool, not an incident management system. AlertOps integrates with New Relic to create a complete system that detects incidents and immediately alerts your incident response teams if there’s a problem.

Here’s how AlertOps and New Relic work together to reduce incident resolution times:

    Automatically route webhook notifications to the right team members. The team members who can resolve the problem and key stakeholders are immediately informed so you get the shortest incident response times.
    Attach custom messages to alerts so team members immediately have the information they need to start troubleshooting.
    Route alerts to other systems for automated incident escalation according to incident escalation policies.
    Automatically open and close alerts based on the webhook received from New Relic to minimize repetitive tasks and eliminate redundant work.

All of this and more is possible with a simple integration using the Generic REST API.

Learn more about AlertOps and New Relic integration and configuration in our help center.
